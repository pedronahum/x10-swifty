...
func cacheIsPerDeviceByExecutableIdentity() async throws {
  ...
  // Same device + same shapes => cache hit => same Executable.identity
  let eCPU1 = try await JIT.compileCached(m, with: be, options: cpu)
  let eCPU2 = try await JIT.compileCached(m, with: be, options: cpu)
  #expect(eCPU1 == eCPU2)

  // Same GPU device twice => hit again
  let eGPU1 = try await JIT.compileCached(m, with: be, options: gpu)
  let eGPU2 = try await JIT.compileCached(m, with: be, options: gpu)
  #expect(eGPU1 == eGPU2)

  // Different devices => different cache entries/Executable identities
  #expect(eCPU1 != eGPU1)
}
